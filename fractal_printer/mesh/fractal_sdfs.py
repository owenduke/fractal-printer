...
def polynomial_update(z, C):
    ...
    zp_1 = order * np.power(z, order-1)
    for i, c in enumerate(C):
        z_1 = z_1 + np.power(z, order-i-1) * c
        zp_1 = zp_1 + (order-i-1) * np.power(z, order-i-2) * c
    return z_1, zp_1

@d3.sdf3
def general_julia_sdf(update, slice=0, order = 2, iterations = 50, bailout = 10000**2, offset=0, fudge_factor = 0.9):
    ...
    def distance(p):
        ...
        zp2 = mag2(zp)
        dist = np.sqrt(z2/zp2)*np.log(z2)/(2*order)
        dist[~mask] = -1
        return (dist - offset) * fudge_factor
